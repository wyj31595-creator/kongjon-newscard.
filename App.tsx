...
const App: React.FC = () => {
  ...
  // 이미지가 잘리거나 작아 보이지 않도록 높이 비율을 다시 상향 조정했습니다.
  const getImageHeight = () => {
    if (currentIndex === 4) return 'h-[25dvh] sm:h-[35%]'; 
  ...
  return (
    <div className="flex justify-center items-center w-full h-[100dvh] bg-gray-200 font-sans overflow-hidden p-0 sm:p-4">
      ...
      >
        ...
        {/* 텍스트 컨텐츠 영역 */}
        <div className={`flex-1 flex flex-col px-6 min-h-0 overflow-hidden bg-white transition-all ${currentIndex === 4 ? 'pt-3 pb-1' : 'pt-5 pb-3'}`}>
          {currentCard.keyword && currentIndex !== 0 && (
            <div className={`${currentIndex >= 3 ? 'mb-1.5 sm:mb-3' : 'mb-3 sm:mb-5'} flex-shrink-0`}>
          ...
          <div className={`${currentIndex === 0 ? 'mt-1' : ''} flex-shrink-0`}>
            {/* 제목: 강조를 위해 기존 크기 유지 */}
            <h1 className={`${currentIndex === 4 ? 'text-[clamp(1.1rem,3.2vh,1.3rem)]' : 'text-[clamp(1.2rem,4vh,1.45rem)]'} font-[900] text-gray-900 leading-[1.3] mb-2 sm:mb-4 whitespace-pre-wrap tracking-tighter break-keep`}>{currentCard.title}</h1>
          </div>

          <div className={`flex-1 flex flex-col no-scrollbar overflow-y-auto ${currentIndex >= 3 ? 'space-y-1.5 sm:space-y-3' : 'space-y-2 sm:space-y-2.5'}`}>
            {/* 소제목: 제목과 분리하기 위해 크기 하향 조정 */}
            ...
            {/* 본문: 가독성을 지키면서 제목보다 작게 설정 (약 16~18px 수준) */}
            {currentCard.body && <p className="text-gray-600 text-[clamp(0.85rem,2.2vh,1.05rem)] leading-[1.6] font-bold whitespace-pre-wrap break-keep tracking-tight">{currentCard.body}</p>}
          ...
          {/* 마지막 페이지 하단 액션 */}
          {isLastPage && (
            <div className="mt-auto pt-1 mb-2 flex-shrink-0 flex flex-col items-center gap-1">
              <button onClick={() => window.open(CARD_NEWS_CONFIG.links.donation, '_blank')} className="w-full max-w-[340px] bg-emerald-500 text-white py-3 sm:py-4 rounded-xl font-black text-[15px] sm:text-[18px] flex flex-col items-center justify-center shadow-md active:scale-95 transition-transform">
                <div className="flex items-center gap-2"><Heart className="w-4 h-4 fill-current" /><span>지금 정기 후원하기</span></div>
                <span className="text-[9px] sm:text-[11px] font-medium mt-0.5 opacity-90">발달장애인의 내일을 지원해주세요</span>
              </button>
              <button onClick={handleShare} className="flex items-center justify-center gap-1.5 text-emerald-600 font-bold text-[11px] sm:text-[13px] py-1 opacity-80 active:opacity-100"><Share2 size={12} /><span>이 소식 주변에 공유하기</span></button>
            </div>
          )}
        </div>
